...
class WorkerStartCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $this->commandBus->dispatch(new StartWorkerCommand($workerName));
        $worker = $this->workerRepository->findByName($workerName);

        $io->success(sprintf('%d instances of worker `%s` are running.', $worker->instancesNumber, $worker->name));
